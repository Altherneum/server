...
# Update + APT
apt update
apt install -y mysql-server phpmyadmin screen vsftpd openjdk-21-jdk curl wget unzip php php-fpm php-mysql php-pgsql php-sqlite3 php-bcmath php-mbstring php-xml php-curl php-zip php-gd apache2 sendmail sendmail-cf mailutils opendkim opendkim-tools fail2ban git git-lfs jq maven
